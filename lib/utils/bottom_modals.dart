...
class Modals {
  ...
  static showCreateplaylistModal(BuildContext context, {Map? item}) {
    ...
    showModalBottomSheet(
      useRootNavigator: true,
      backgroundColor: const Color.fromARGB(187, 143, 0, 252),
      useSafeArea: true,
      isScrollControlled: true,
      context: context,
      builder: (context) => _createPlaylistModal(title, context, item),
  ...
  static showImportplaylistModal(BuildContext context, {Map? item}) {
    if (Platform.isWindows) {
      return fluent_ui.showDialog(
        context: context,
        useRootNavigator:
            true, // Changed to true to ensure it's above all navigation
        barrierDismissible: true,
    ...
    showModalBottomSheet(
      ...
      builder: (context) => Wrap(
        children: [
          SafeArea(
            child: _importPlaylistModal(context),
          ),
        ],
      ),
    );
            ...
            AdaptiveFilledButton(
              onPressed: () {
                ...
                if (updateInfo != null) {
                  ...
                  launchUrl(
                      Uri.parse(
                          'https://are-music.vercel.app/api/download?action=download&platform=$platform&url=${updateInfo.downloadUrl}'),
                      mode: LaunchMode.externalApplication);
                }
              },
